...
class ECBS {
 public:
  ...
  bool search(const std::vector<State>& initialStates,
    ...
    // compute the max g value
    start.max_g = computeMaxG(start.solution);
...
// check focal list/open list consistency
      ...
      for (const auto& c : constraints) {
        ...
        if (!newNode.constraints[i].overlap(c.second)) {
          std::cout << "Error! This constraint was included already!" << std::endl;
          return false;
        }
        ...
        // compute the max gvalue
        newNode.max_g = computeMaxG(newNode.solution);
...
// boost::heap::compare<compareFocalHeuristic> > focalSet_t;
  ...
  struct HighLevelNode {
    ...
    Cost focalHeuristic;
    // store the max gvalue among all the robots
    Cost max_g;
    ...
    friend std::ostream& operator<<(std::ostream& os, const HighLevelNode& c) {
      ...
      os << "id: " << c.id << " max_g: " << c.max_g << " cost: " << c.cost << " LB: " << c.LB << " focal: " << c.focalHeuristic << std::endl;
  ...
  struct compareFocalHeuristic {
    bool operator()(const handle_t& h1, const handle_t& h2) const {
      ...
      // }
      if ((*h1).max_g != (*h2).max_g) {
        return (*h1).max_g > (*h2).max_g;  
      }
 ...
 private:
  ...
  static Cost computeMaxG(const std::vector<PlanResult<State, Action, Cost> >& solution) {
    Cost max_g = 0;
    for (const auto& s : solution) {
      if (!s.states.empty()) {
        max_g = std::max(max_g, static_cast<Cost>(s.states.size()));
      }
    }
    return max_g;
  }
