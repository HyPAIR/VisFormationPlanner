...
#include <to_solver/optimization.h>

// Set the timestep length
size_t N;
int qn;
std::vector<std::vector<std::vector<double>>> plan(3, std::vector<std::vector<double>>(3000, std::vector<double>(7)));
...
private:
    ...
    bool updateObsBox(bool log){
        ...
        std::vector<std::vector<std::vector<double>>> plan(qn, std::vector<std::vector<double>>(5 * M + 1, std::vector<double>(3)));
        int count=0;
