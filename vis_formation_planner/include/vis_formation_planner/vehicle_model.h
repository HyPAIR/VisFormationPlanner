  ...
  */
  ...
  std::vector<T> GetVertexPositions(const T &x, const T &y, const T &theta, const T &inflat) const {
    ...
    if (inflat > 1) {
      ...
      result.push_back(vert_l_x + (1.2 + 3.0) * cos(theta));
      result.push_back(vert_l_y + (1.2 + offset_) * sin(theta));
      result.push_back(vert_l_x + hypot(1.2 + 3.0, 1.2 + offset_) * cos(theta - atan2(1.2 + offset_, 1.2 + 3.0)));
      result.push_back(vert_l_y + hypot(1.2 + 3.0, 1.2 + offset_) * sin(theta - atan2(1.2 + offset_, 1.2 + 3.0)));
...
public:
  ...
  double formation_radius = 4.05 / sqrt(3);
  double radius_inc = 0.2;
  double xv1 = 0.0;
  double yv1 = 0.0;
  // double xv2 = 3.3;
  double xv2 = formation_radius * sqrt(3);
  double yv2 = 0.0;
  double xv3 = xv2 / 2;
