...
public:
  ...
  bool Plan(math::Pose start, math::Pose goal, std::vector<math::Pose> &result, const std::vector<std::vector<double>> corridor_sets);
...
private:
  ...
  struct Node2d {
    ...
    inline math::Vec2d Generate2DPoint(math::Vec2d origin, const PlannerConfig &config) const {
      math::Vec2d corner(origin.x() + config.grid_xy_resolution * (x_grid + ceil(config.vehicle.offset / 2)), origin.y() + config.grid_xy_resolution * (y_grid + ceil(config.vehicle.offset / 2)));
      return corner;
    }

    inline math::AABox2d GenerateBoxOffset(math::Vec2d origin, const PlannerConfig &config) const {
      math::Vec2d corner(origin.x() + config.grid_xy_resolution * (x_grid + ceil(config.vehicle.offset / 2)), origin.y() + config.grid_xy_resolution * (y_grid + ceil(config.vehicle.offset / 2)));
      return { corner, 2.0 + 1.2, config.vehicle.offset + 1.2 };
    }
  ...
  bool ExpandNextNode(const Node3d &node, int next_index, Node3d &next_node, const std::vector<std::vector<double>> corridor_sets);

  double EvaluateExpandCost(const Node3d &parent, const Node3d &node);

  double EstimateHeuristicCost(const Node3d &node, const std::vector<std::vector<double>> corridor_sets);

  double Calculate2DCost(const Node3d &node, const std::vector<std::vector<double>> corridor_sets);

  bool CheckOneshotPath(const Node3d &node, const Node3d &goal, std::vector<math::Pose> &result, const std::vector<std::vector<double>> corridor_sets);
