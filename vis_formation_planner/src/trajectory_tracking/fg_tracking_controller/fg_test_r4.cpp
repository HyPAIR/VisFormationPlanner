...
geometry_msgs::Twist robot4_cmd;
ros::Publisher robot4_cmd_pub; // node publishers
tf::Point Odom_pos;    //odometry position (x,y)
geometry_msgs::Pose actual_pose;
double Odom_yaw;    //odometry orientation (yaw)
geometry_msgs::Pose2D qd,robot_pose4, err;
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "robot_control_4");
    ros::NodeHandle n;
    ros::Subscriber sub_odometry_robot1 = n.subscribe("/mir4/mir_pose_simple", 1000 , robot_actual_pose4);
	...
	while(path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (hypot(robot_pose4.x - plan[robot_index].back()[0], robot_pose4.y - plan[robot_index].back()[1]) < 0.1 && fabs(robot_pose4.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
		...
		if (actual_pose.position.x == 0 && actual_pose.position.y == 0) {
			robot4_cmd.linear.x = 0.0;
			robot4_cmd.angular.z = 0.0;			
		}
		else {
			robot4_cmd.linear.x = target_vels;
			robot4_cmd.angular.z = target_angvels;
			auto cmd = cartesian_controller(actual_pose, target_pose_x, target_pose_y, target_pose_theta, target_vels, target_angvels);
			robot4_cmd.linear.x = cmd[0];
			robot4_cmd.angular.z = cmd[1];
		}
		
		if (robot_pose4.x != 0 && robot_pose4.y != 0 && robot_pose4.theta != 0) {
			x1_plot.push_back(robot_pose4.x);
			y1_plot.push_back(robot_pose4.y);
			theta1_plot.push_back(robot_pose4.theta);
		}
		robot4_cmd_pub.publish(robot4_cmd);
		ros::spinOnce();
		loop_rate.sleep();
	}
	// plt::clf();
    // plot_traj(plan[robot_index], x1_plot, y1_plot, theta1_plot, obstacle, "Diff4 trajectory");
	// plt::show();
	std::cout << "robot 4 done!" << std::endl;
    return 0;
}