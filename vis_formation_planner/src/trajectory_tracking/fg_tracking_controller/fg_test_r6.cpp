...
geometry_msgs::Twist robot6_cmd;
ros::Publisher robot6_cmd_pub, robot6_vel_pub; // node publishers
tf::Point Odom_pos;    //odometry position (x,y)
double Odom_yaw;    //odometry orientation (yaw)
geometry_msgs::Pose2D qd,robot_pose6, err, actual_pose;
...
void robot_model_ind_call6(const gazebo_msgs::ModelStates::ConstPtr& msg) {
...
void robot_odom6(const gazebo_msgs::ModelStates::ConstPtr& msg) {
	...
	robot_pose6.x = msg->pose[robot_model_index].position.x;
	robot_pose6.y = msg->pose[robot_model_index].position.y;
...
int main(int argc, char **argv)
{
	...
	// int robot_index = 0;
	// auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/vis_formation_planner/traj_result/ras_demo/result/leader1.yaml");
	...
	// initialization of ROS node
    ros::init(argc, argv, "robot_control_6");
    ros::NodeHandle n;
	ros::Subscriber sub_robot_model_ind_5 = n.subscribe("gazebo/model_states", 1000, robot_model_ind_call6);
 	ros::Subscriber sub_pose_robot5 = n.subscribe("gazebo/model_states", 1000, robot_odom6);
	robot6_cmd_pub = n.advertise<geometry_msgs::Twist>("/hunter2_base2/ackermann_steering_controller/cmd_vel", 1000);
	...
	while (path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (robot_pose6.x != 0 && robot_pose6.y != 0) {
		...
		if (hypot(robot_pose6.x - plan[robot_index].back()[0], robot_pose6.y - plan[robot_index].back()[1]) < 0.1 && fabs(robot_pose6.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
		...
		timestamp_old = ros::Time::now();
		if (robot_pose6.x == 0 && robot_pose6.y == 0) {
			robot6_cmd.linear.x = 0.0;
			robot6_cmd.angular.z = 0.0;			
		}
		else {
			robot6_cmd.linear.x = target_vels;
			robot6_cmd.angular.z = target_angvels;
			auto cmd = cartesian_controller(robot_pose6, target_pose_x, target_pose_y, target_pose_theta, target_vels, target_angvels);
			robot6_cmd.linear.x = cmd[0];
			robot6_cmd.angular.z = cmd[1];
		}
		robot6_cmd_pub.publish(robot6_cmd);
		if (robot_pose6.x != 0 && robot_pose6.y != 0 && robot_pose6.theta != 0) {
			x6_plot.push_back(robot_pose6.x);
			y6_plot.push_back(robot_pose6.y);
			theta6_plot.push_back(robot_pose6.theta);
		}
		ros::spinOnce();
		loop_rate.sleep();
	};
	// plt::clf();
    // plot_traj(plan[robot_index], x6_plot, y6_plot, theta6_plot, obstacle, "Car2 trajectory");
	// plt::show();
	std::cout << "robot 6 done!" << std::endl;
    return 0;
}