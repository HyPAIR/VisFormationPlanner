...
int main(int argc, char **argv)
{
	...
	// int robot_index = 0;
	// auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/vis_formation_planner/traj_result/ras_demo/result/diff_drive1.yaml");
    ...
    ros::init(argc, argv, "robot_control_2");
    ros::NodeHandle n;
    ros::Subscriber sub_odometry_robot1 = n.subscribe("/mir2/mir_pose_simple", 1000 , robot_actual_pose2);
	...
	while(path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (hypot(robot_pose2.x - plan[robot_index].back()[0], robot_pose2.y - plan[robot_index].back()[1]) < 0.1 && fabs(robot_pose2.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
		...
		if (actual_pose.position.x == 0 && actual_pose.position.y == 0) {
			robot2_cmd.linear.x = 0.0;
			robot2_cmd.angular.z = 0.0;			
		}
		else {
			robot2_cmd.linear.x = target_vels;
			robot2_cmd.angular.z = target_angvels;
			auto cmd = cartesian_controller(actual_pose, target_pose_x, target_pose_y, target_pose_theta, target_vels, target_angvels);
			robot2_cmd.linear.x = cmd[0];
			robot2_cmd.angular.z = cmd[1];
		}
		
		if (robot_pose2.x != 0 && robot_pose2.y != 0 && robot_pose2.theta != 0) {
			x1_plot.push_back(robot_pose2.x);
			y1_plot.push_back(robot_pose2.y);
			theta1_plot.push_back(robot_pose2.theta);
		}
		robot2_cmd_pub.publish(robot2_cmd);
		ros::spinOnce();
		loop_rate.sleep();
	}
	std::cout << "robot 2 done!" << std::endl;
	// plt::clf();
    // plot_traj(plan[robot_index], x1_plot, y1_plot, theta1_plot, obstacle, "Diff2 trajectory");
	// plt::show();
    return 0;
}