...
geometry_msgs::Twist robot8_cmd;
ros::Publisher robot8_cmd_pub, robot8_vel_pub; // node publishers
tf::Point Odom_pos;    //odometry position (x,y)
double Odom_yaw;    //odometry orientation (yaw)
geometry_msgs::Pose2D qd,robot_pose8, err, actual_pose;
...
void robot_model_ind_call8(const gazebo_msgs::ModelStates::ConstPtr& msg) {
...
void robot_odom8(const gazebo_msgs::ModelStates::ConstPtr& msg) {
	Odom_yaw = tf::getYaw(msg->pose[robot_model_index].orientation);
	robot_pose8.x = msg->pose[robot_model_index].position.x;
	robot_pose8.y = msg->pose[robot_model_index].position.y;
...
int main(int argc, char **argv)
{
	...
	// int robot_index = 0;
	// auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/vis_formation_planner/traj_result/ras_demo/result/car_like0.yaml");
	...
	// initialization of ROS node
    ros::init(argc, argv, "robot_control_8");
    ros::NodeHandle n;
	ros::Subscriber sub_robot_model_ind_5 = n.subscribe("gazebo/model_states", 1000, robot_model_ind_call8);
 	ros::Subscriber sub_pose_robot5 = n.subscribe("gazebo/model_states", 1000, robot_odom8);
	robot8_cmd_pub = n.advertise<geometry_msgs::Twist>("/hunter2_base4/ackermann_steering_controller/cmd_vel", 1000);
	...
	while (path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (robot_pose8.x != 0 && robot_pose8.y != 0) {
		...
		if (hypot(robot_pose8.x - plan[robot_index].back()[0], robot_pose8.y - plan[robot_index].back()[1]) < 0.1 && fabs(robot_pose8.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
		...
		timestamp_old = ros::Time::now();
		if (robot_pose8.x == 0 && robot_pose8.y == 0) {
			robot8_cmd.linear.x = 0.0;
			robot8_cmd.angular.z = 0.0;			
		}
		else {
			robot8_cmd.linear.x = target_vels;
			robot8_cmd.angular.z = target_angvels;
			auto cmd = cartesian_controller(robot_pose8, target_pose_x, target_pose_y, target_pose_theta, target_vels, target_angvels);
			robot8_cmd.linear.x = cmd[0];
			robot8_cmd.angular.z = cmd[1];
		}
		robot8_cmd_pub.publish(robot8_cmd);
		if (robot_pose8.x != 0 && robot_pose8.y != 0 && robot_pose8.theta != 0) {
			x8_plot.push_back(robot_pose8.x);
			y8_plot.push_back(robot_pose8.y);
			theta8_plot.push_back(robot_pose8.theta);
		}
		ros::spinOnce();
		loop_rate.sleep();
	};
	// plt::clf();
    // plot_traj(plan[robot_index], x8_plot, y8_plot, theta8_plot, obstacle, "Car4 trajectory");
	// plt::show();
	std::cout << "robot 8 done!" << std::endl;
    return 0;
}