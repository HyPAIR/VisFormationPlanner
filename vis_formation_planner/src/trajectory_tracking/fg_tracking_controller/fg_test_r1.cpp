...
int main(int argc, char **argv)
{
	...
	// int robot_index = 0;
	// auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/vis_formation_planner/traj_result/ras_demo/result/diff_drive0.yaml");
	...
	while(path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (hypot(robot_pose1.x - plan[robot_index].back()[0], robot_pose1.y - plan[robot_index].back()[1]) < 0.1 && fabs(robot_pose1.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
	...
	for (int j =0; j < plan[0].size(); j++) {
		for (int i = 0; i < x1_plot.size(); i++) {
			double temp_error = hypot(plan[robot_index][j][0] - x1_plot[i], plan[robot_index][j][1] - y1_plot[i]);
	...
	// plt::show();
	std::cout << "robot 1 done!" << std::endl;
	return 0;
}