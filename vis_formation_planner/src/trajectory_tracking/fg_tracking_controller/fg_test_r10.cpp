...
geometry_msgs::Twist robot10_cmd;
ros::Publisher robot10_cmd_pub, robot10_vel_pub; // node publishers
tf::Point Odom_pos;    //odometry position (x,y)
double Odom_yaw;    //odometry orientation (yaw)
geometry_msgs::Pose2D qd,robot_pose10, err, actual_pose;
...
void robot_model_ind_call10(const gazebo_msgs::ModelStates::ConstPtr& msg) {
...
void robot_odom10(const gazebo_msgs::ModelStates::ConstPtr& msg) {
	Odom_yaw = tf::getYaw(msg->pose[robot_model_index].orientation);
	robot_pose10.x = msg->pose[robot_model_index].position.x;
	robot_pose10.y = msg->pose[robot_model_index].position.y;
...
int main(int argc, char **argv)
{
	...
	// int robot_index = 0;
	// auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/vis_formation_planner/traj_result/ras_demo/result/car_like2.yaml");
	...
	// initialization of ROS node
    ros::init(argc, argv, "robot_control_10");
    ros::NodeHandle n;
	ros::Subscriber sub_robot_model_ind_5 = n.subscribe("gazebo/model_states", 1000, robot_model_ind_call10);
 	ros::Subscriber sub_pose_robot5 = n.subscribe("gazebo/model_states", 1000, robot_odom10);
	robot10_cmd_pub = n.advertise<geometry_msgs::Twist>("/hunter2_base6/ackermann_steering_controller/cmd_vel", 1000);
	...
	while (path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (robot_pose10.x != 0 && robot_pose10.y != 0) {
		...
		if (hypot(robot_pose10.x - plan[robot_index].back()[0], robot_pose10.y - plan[robot_index].back()[1]) < 0.1 && fabs(robot_pose10.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
		...
		timestamp_old = ros::Time::now();
		if (robot_pose10.x == 0 && robot_pose10.y == 0) {
			robot10_cmd.linear.x = 0.0;
			robot10_cmd.angular.z = 0.0;			
		}
		else {
			robot10_cmd.linear.x = target_vels;
			robot10_cmd.angular.z = target_angvels;
			auto cmd = cartesian_controller(robot_pose10, target_pose_x, target_pose_y, target_pose_theta, target_vels, target_angvels);
			robot10_cmd.linear.x = cmd[0];
			robot10_cmd.angular.z = cmd[1];
		}
		robot10_cmd_pub.publish(robot10_cmd);
		if (robot_pose10.x != 0 && robot_pose10.y != 0 && robot_pose10.theta != 0) {
			x10_plot.push_back(robot_pose10.x);
			y10_plot.push_back(robot_pose10.y);
			theta10_plot.push_back(robot_pose10.theta);
		}
		ros::spinOnce();
		loop_rate.sleep();
	};
	// plt::clf();
    // plot_traj(plan[robot_index], x10_plot, y10_plot, theta10_plot, obstacle, "Car6 trajectory");
	// plt::show();
	std::cout << "robot 10 done!" << std::endl;
    return 0;
}