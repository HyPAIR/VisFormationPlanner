...
geometry_msgs::Twist robot9_cmd;
ros::Publisher robot9_cmd_pub, robot9_vel_pub; // node publishers
tf::Point Odom_pos;    //odometry position (x,y)
double Odom_yaw;    //odometry orientation (yaw)
geometry_msgs::Pose2D qd,robot_pose9, err, actual_pose;
...
void robot_model_ind_call9(const gazebo_msgs::ModelStates::ConstPtr& msg) {
...
void robot_odom9(const gazebo_msgs::ModelStates::ConstPtr& msg) {
	Odom_yaw = tf::getYaw(msg->pose[robot_model_index].orientation);
	robot_pose9.x = msg->pose[robot_model_index].position.x;
	robot_pose9.y = msg->pose[robot_model_index].position.y;
...
int main(int argc, char **argv)
{
	...
	// int robot_index = 0;	
	// auto traj_1 = generate_traj("/home/weijian/Heterogeneous_formation/src/vis_formation_planner/traj_result/ras_demo/result/car_like1.yaml");
	...
	// initialization of ROS node
    ros::init(argc, argv, "robot_control_9");
    ros::NodeHandle n;
	ros::Subscriber sub_robot_model_ind_5 = n.subscribe("gazebo/model_states", 1000, robot_model_ind_call9);
 	ros::Subscriber sub_pose_robot5 = n.subscribe("gazebo/model_states", 1000, robot_odom9);
	robot9_cmd_pub = n.advertise<geometry_msgs::Twist>("/hunter2_base5/ackermann_steering_controller/cmd_vel", 1000);
	...
	while (path_index < plan[robot_index].size() - 2 && ros::ok()){
		...
		if (robot_pose9.x != 0 && robot_pose9.y != 0) {
		...
		if (hypot(robot_pose9.x - plan[robot_index].back()[0], robot_pose9.y - plan[robot_index].back()[1]) < 0.1 && fabs(robot_pose9.theta - plan[robot_index].back()[2]) < 0.1) {
			break;
		}
		distances_old = distances;
		...
		timestamp_old = ros::Time::now();
		if (robot_pose9.x == 0 && robot_pose9.y == 0) {
			robot9_cmd.linear.x = 0.0;
			robot9_cmd.angular.z = 0.0;			
		}
		else {
			robot9_cmd.linear.x = target_vels;
			robot9_cmd.angular.z = target_angvels;
			auto cmd = cartesian_controller(robot_pose9, target_pose_x, target_pose_y, target_pose_theta, target_vels, target_angvels);
			robot9_cmd.linear.x = cmd[0];
			robot9_cmd.angular.z = cmd[1];
		}
		robot9_cmd_pub.publish(robot9_cmd);
		if (robot_pose9.x != 0 && robot_pose9.y != 0 && robot_pose9.theta != 0) {
			x9_plot.push_back(robot_pose9.x);
			y9_plot.push_back(robot_pose9.y);
			theta9_plot.push_back(robot_pose9.theta);
		}
		ros::spinOnce();
		loop_rate.sleep();
	};
	...
	double temp_error_ = 1e5;
	double old_error = 0.0;
	for (int j =0; j < plan[0].size(); j++) {
		for (int i = 0; i < x9_plot.size(); i++) {
			double temp_error = hypot(plan[robot_index][j][0] - x9_plot[i], plan[robot_index][j][1] - y9_plot[i]);
			if (temp_error < temp_error_)
				temp_error_ = temp_error;
		}
		old_error = temp_error_;
		if (old_error < temp_error_)
			max_error = temp_error_;
		avg_error.push_back(temp_error_);
	}	
	double sum_error = 0;
	for (int i = 0; i < avg_error.size(); i++) {
		sum_error += avg_error[i];
		if (avg_error[i] > max_error)
			max_error = avg_error[i];
	}
	// plt::clf();
    // plot_traj(plan[robot_index], x9_plot, y9_plot, theta9_plot, obstacle, "Car5 trajectory");
	// plt::show();
	std::cout << "robot 9 done!" << std::endl;
    return 0;
}