...
void plot_formation_linear_error(std::vector<double> t, Trajectory traj_1, Trajectory traj_fol1, Trajectory traj_fol_diff_r1, Trajectory traj_fol_diff_r2) {
	std::vector<double> error_12, error_13, error_14;
	for (int i = 0; i < traj_1.size(); i++) {
		error_12.push_back(hypot(traj_1[i].x - traj_fol1[i].x, traj_1[i].y - traj_fol1[i].y) - 2.016733665);
		error_13.push_back(hypot(traj_1[i].x - traj_fol_diff_r1[i].x, traj_1[i].y - traj_fol_diff_r1[i].y) - 2);
		error_14.push_back(hypot(traj_1[i].x - traj_fol_diff_r2[i].x, traj_1[i].y - traj_fol_diff_r2[i].y) - hypot(2, 2.016733665));
	}
    std::map<std::string, std::string> style1, style2, style3;      
	style1 = {{"color", "blue"}, {"label", "Diff-drive robot 2"}};
	style2 = {{"color", "red"}, {"label", "Car-like robot 1"}};
	style3 = {{"color", "green"}, {"label", "Car-like robot 2"}};
	plt::clf();
	plt::plot(t, error_12, style1);
	plt::plot(t, error_13, style2);
	plt::plot(t, error_14, style3);
	plt::xlabel("t");
...
void plot_formation_angular_error(std::vector<double> t, Trajectory_temp traj_1, Trajectory_temp traj_fol1, Trajectory_temp traj_fol_diff_r1, Trajectory_temp traj_fol_diff_r2) {
	std::vector<double> error_12, error_13, error_14;
	for (int i = 0; i < traj_1.size(); i++) {
		error_12.push_back(traj_1[i].theta - traj_fol1[i].theta);
		error_13.push_back(traj_1[i].theta - traj_fol_diff_r1[i].theta);
		error_14.push_back(traj_1[i].theta - traj_fol_diff_r2[i].theta);
	}
    std::map<std::string, std::string> style1, style2, style3;      
	style1 = {{"color", "blue"}, {"label", "Diff-drive robot 2"}};
	style2 = {{"color", "red"}, {"label", "Car-like robot 1"}};
...
int main(int argc, char **argv)
{
	...
	auto traj_1 = generate_traj("/home/weijian/liom_test_ws/src/vis_formation_planner/traj_test_3.yaml");
	...
	auto traj_fol_diff_r1 = generate_traj("/home/weijian/liom_test_ws/src/vis_formation_planner/traj_test_diff4_1.yaml");
	auto traj_fol_diff_r2 = generate_traj("/home/weijian/liom_test_ws/src/vis_formation_planner/traj_test_diff4_2.yaml");
	for (int i = 0; i < traj_1.size(); i++) {
	...
	for (int i = 0; i < traj_fol_diff_r2.size(); i++) {
		v_diff2.push_back(traj_fol_diff_r2[i].v);
		a_diff2.push_back(traj_fol_diff_r2[i].a);
		w_diff2.push_back(traj_fol_diff_r2[i].phi);
		omega_diff2.push_back(traj_fol_diff_r2[i].omega);
	}
    // plot_v_diff(t_set, v_diff1, v_diff2);
    // plot_a_diff(t_set, a_diff1, a_diff2);
    // plot_omega_diff(t_set, omega_diff1, omega_diff2);
    // plot_w_diff(t_set, w_diff1, w_diff2);
	...
	// plot_formation_linear_error(t_set, traj_1, traj_fol1, traj_fol_diff_r1, traj_fol_diff_r2);
	plot_formation_angular_error(t_set, traj_1, traj_fol1, traj_fol_diff_r1, traj_fol_diff_r2);
	...
	for (i = 0; i < x4_orig_plot.size(); i++) {
		plot_car(x4_orig_plot[i], y4_orig_plot[i], theta4_orig_plot[i], 2, i);
	}
