...
#include "vis_formation_planner/math/math_utils.h"
...
bool CoarsePathPlanner::Plan(math::Pose start, math::Pose goal, std::vector<math::Pose> &result, const std::vector<std::vector<double>> corridor_sets) {
  ...
  start_node.set_cost(0, EstimateHeuristicCost(start_node, corridor_sets));
  ...
  while(!open_pq_.empty()) {
    ...
    // 检查是否可以一步到达
    if(should_check && CheckOneshotPath(node, goal_node, oneshot_path, corridor_sets)) {
    ...
    for(int i = 0; i < next_node_num; i++) {
      Node3d next_node;
      if(!ExpandNextNode(node, i, next_node, corridor_sets)) {
        continue;
      }
      ...
      // g(non-holonomic without-obstacles) + h(holonomic with obstacles)
      next_node.set_cost(
          node.g_cost + EvaluateExpandCost(node, next_node),
          EstimateHeuristicCost(next_node, corridor_sets));
...
double CoarsePathPlanner::EstimateHeuristicCost(const Node3d &node, const std::vector<std::vector<double>> corridor_sets) {
  return Calculate2DCost(node, corridor_sets);
}
...
double CoarsePathPlanner::Calculate2DCost(const Node3d &node_3d, const std::vector<std::vector<double>> corridor_sets) {
  ...
  // if(env_->CheckBoxCollision(0.0, node_2d.GenerateBoxOffset(origin_, *config_))) {
    return inf;
  }
  // auto temp_2d_node = node_2d.Generate2DPoint(origin_, *config_);
  // math::Pose temp_3d_node(temp_2d_node.x(), temp_2d_node.y(), 0.0);
  // if (!env_->CheckHomotopyConstraints(0.0, temp_3d_node, corridor_sets)) {
  //   return inf;
  // }
  while(!grid_open_pq_.empty()) {
    ...
    for(int i = 0; i < 8; i++) {
      ...
      if(env_->CheckBoxCollision(0.0, expand_node.GenerateBox(origin_, *config_))
        // || !env_->CheckHomotopyConstraints(0.0, temp_3d_node, corridor_sets)
      ) {
      // if(env_->CheckBoxCollision(0.0, expand_node.GenerateBoxOffset(origin_, *config_)) 
        // || !env_->CheckHomotopyConstraints(0.0, temp_3d_node, corridor_sets)
        // ) {
...
bool CoarsePathPlanner::ExpandNextNode(const Node3d &node, int next_index, Node3d &next_node, const std::vector<std::vector<double>> corridor_sets) {
  double steering;
  ...
  if (is_forward) {
    ...
    steering = -config_->vehicle.phi_min + 2 * config_->vehicle.phi_max / node_res * index;
  }
  ...
  for(auto &pose: path) {
    ...
    // if(!env_->CheckHomotopyConstraints(0.0, pose, corridor_sets)) {
    //   return false;
    // }
  }
  // if(!env_->CheckHomotopyConstraints(0.0, path[path.size() - 1], corridor_sets)) {
  //   return false;
  // }
...
bool CoarsePathPlanner::CheckOneshotPath(const Node3d &node, const Node3d &goal, std::vector<math::Pose> &result, std::vector<std::vector<double>> corridor_sets) {
  ...
  // if(!env_->CheckHomotopyConstraints(0.0, result[result.size() - 1], corridor_sets)) {
  //   return false;
  // }
  return true;
}
