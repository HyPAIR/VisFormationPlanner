...
#include "vis_formation_planner/lightweight_nlp_problem.h"
#include "vis_formation_planner/time.h"
...
class LiomIPOPTInterfaceFm : public Ipopt::TNLP
{
public:
  LiomIPOPTInterfaceFm(double w_inf, const std::vector<Constraints> &profile, const std::vector<FullStates> &guess, const PlannerConfig &config, const std::vector<double>& height_cons)
  ...
  virtual ~LiomIPOPTInterfaceFm() = default;

  LiomIPOPTInterfaceFm(const LiomIPOPTInterfaceFm&) = delete;
  LiomIPOPTInterfaceFm& operator=(const LiomIPOPTInterfaceFm&) = delete;
  ...
  virtual bool get_bounds_info(Ipopt::Index n, Ipopt::Number* x_l, Ipopt::Number* x_u,
    ...
    for (int i = 0; i < nrows_; i++) {
      ...
      if (height_cons_[i + 1] == -1) {
        ...
        for (int j = 0; j < num_robot; ++j) {
          lb_vec_(j) = sqrt(3) * height_cons_[i + 1] * sqrt(3) * height_cons_[i + 1];
        }
        for (int j = num_robot; j < num_robot + num_robot * (num_robot - 2); ++j) {
            lb_vec_(j) = 0.0;
        }       
  ...
  template<class T> T eval_infeasibility(const T *x) {
    ...
    T infeasibility_terminal_x = 0.0;
    T infeasibility_terminal_y = 0.0;
    T infeasibility_target_x = 0.0;
    T infeasibility_target_y = 0.0;
    // initial constraints
    for (int j = 0; j < num_robot; j++) {
      infeasibility_terminal_x += x[1 + (0 + 7 * j) * nrows_ + nrows_ - 1];
      infeasibility_terminal_y += x[1 + (1 + 7 * j) * nrows_ + nrows_ - 1];
      infeasibility_target_x += profile_[j].goal.x;
      infeasibility_target_y += profile_[j].goal.y;
    }
    for (int j = 0; j < num_robot; j++) {
      ...
      infeasibility_terminal += pow((infeasibility_terminal_x - infeasibility_target_x) / num_robot, 2)
                              + pow((infeasibility_terminal_y - infeasibility_target_y) / num_robot, 2);
  ...
  template<class T> bool eval_obj(Ipopt::Index n, const T *x, T& obj_value) {
    ...
    // for (int j = 0; j < num_robot; j++) {
    //   for(int i = 0; i < nrows_; i++) {
    //     obj_value += config_.opti_w_a * pow(((x[1 + (0 + 7 * ((j + 1) % num_robot)) * nrows_ + i] - x[1 + (0 + 7 * j) * nrows_ + i]) * (x[1 + (0 + 7 * ((j + 1) % num_robot)) * nrows_ + i] - x[1 + (0 + 7 * j) * nrows_ + i]) + 
    //                     (x[1 + (1 + 7 * ((j + 1) % num_robot)) * nrows_ + i] - x[1 + (1 + 7 * j) * nrows_ + i]) * (x[1 + (1 + 7 * ((j + 1) % num_robot)) * nrows_ + i] - x[1 + (1 + 7 * j) * nrows_ + i]) - vvcm.xv2t * vvcm.xv2t), 2);
    //   }
    // }
    // for (int i = 0; i < nrows_; i++) {
...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
class LiomIPOPTInterface_diff_drive : public Ipopt::TNLP
{
public:
  LiomIPOPTInterface_diff_drive(double w_inf, const Constraints &profile, const FullStates &guess, const FullStates &pre_sol, const PlannerConfig &config)
  ...
  virtual ~LiomIPOPTInterface_diff_drive() = default;

  LiomIPOPTInterface_diff_drive(const LiomIPOPTInterface_diff_drive&) = delete;
  LiomIPOPTInterface_diff_drive& operator=(const LiomIPOPTInterface_diff_drive&) = delete;
...
class LiomIPOPTInterface_car_like_replan : public Ipopt::TNLP
{
public:
  LiomIPOPTInterface_car_like_replan(double w_inf, const Constraints &profile, const FullStates &guess, const FullStates &pre_sol, const PlannerConfig &config)
    : w_inf_(w_inf), profile_(profile), guess_(guess), pre_sol_(pre_sol), config_(config) {
    nfe_ = guess_.states.size();
    nrows_ = nfe_ - 2;
    // disc_nvar_ = config_.vehicle.n_disc * 2;
  ...
  virtual ~LiomIPOPTInterface_car_like_replan() = default;

  LiomIPOPTInterface_car_like_replan(const LiomIPOPTInterface_car_like_replan&) = delete;
  LiomIPOPTInterface_car_like_replan& operator=(const LiomIPOPTInterface_car_like_replan&) = delete;
...
bool LightweightProblem::Solve_diff_drive(
  ...
  Ipopt::SmartPtr<LiomIPOPTInterface_diff_drive> interface = new LiomIPOPTInterface_diff_drive(w_inf, profile, guess, pre_sol, *config_);
...
bool LightweightProblem::Solve_replan(
  ...
  Ipopt::SmartPtr<LiomIPOPTInterface_car_like_replan> interface = new LiomIPOPTInterface_car_like_replan(w_inf, profile, guess, pre_sol, *config_);
...
bool LightweightProblem::SolveFm(
  ...
  Ipopt::SmartPtr<LiomIPOPTInterfaceFm> interface = new LiomIPOPTInterfaceFm(w_inf, profile, guess, *config_, height_cons);
